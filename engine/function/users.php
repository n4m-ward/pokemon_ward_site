...
// CREATE ACCOUNT
function getLastIdAccount()
{
	$data = mysql_select_single("SELECT `id` FROM `accounts` WHERE `id` ORDER BY `id` DESC");
	return $data['id'];
}

function getLastPlayer()
{
	$data = mysql_select_single("SELECT `id` FROM `players` WHERE `id` ORDER BY `id` DESC");
	return $data['id'];
}
...
function user_create_account($register_data, $maildata) {
	...
	$lastId = getLastIdAccount() + 1;
	mysql_insert("INSERT INTO `accounts` (`id`, $fields) VALUES ($lastId, $data)");
	
	$account_id = user_id($register_data['name']);
	$activeKey = rand(100000000,999999999);
	$active = ($maildata['register']) ? 0 : 1;
/* 	mysql_insert("INSERT INTO `znote_accounts` (`id`, `account_id`, `ip`, `created`, `active`, `activekey`, `flag`) VALUES ('$account_id', '$account_id', '$ip', '$created', '$active', '$activeKey', '$flag')"); */
...
function user_create_character($character_data) {
	...
	$import_data = array(
		'id' => getLastPlayer() + random_int(6000, 20000),
	...
	$userId = getLastPlayer() + 1;
	/* mysql_insert("INSERT INTO `znote_players`(`id`, `player_id`, `created`, `hide_char`, `comment`) VALUES ('$userId', '$charid', '$created', '0', '');"); */
