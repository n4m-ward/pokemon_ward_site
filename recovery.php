	...
	if (!empty($_POST)) {
		...
		if ($status) {
			if (!$username) {
				...
				$user = mysql_select_single("SELECT `p`.`id` AS `player_id`, `a`.`name` FROM `players` `p` INNER JOIN `accounts` `a` ON `p`.`account_id` = `a`.`id` WHERE `p`.`name` = '$character' AND `a`.`email` = '$email' LIMIT 1;");
				if ($user !== false) {
					// Found user
